...
namespace big
{
	// FIXED: Added 'public' inheritance so the base constructor can be resolved outside the class
	class ped_rain : public looped_command
	{
		...
		virtual void on_tick() override
		{
			...
			if (elapsed_time >= 300)
			{
				...
				// FIXED: Changed from range-for syntax ':' to assignment '='
				auto peds = entity::get_entities(false, true); 
				if (peds.empty())
					return;
